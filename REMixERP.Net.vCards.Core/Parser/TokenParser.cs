...
namespace MixERP.Net.VCards.Parser
{
    public static class TokenParser
    {
        ...
        private static Token GetToken(string line)
        {
            ...
            var keySections = GetKeySections(entry.Key);//Получили секцию ключей из строки
            var additionalKeyMembers = GetAdditionalKeyMembers(keySections[1]);
            if (additionalKeyMembers.Count > 0) entry = additionalKeyMembersProcessing(entry, additionalKeyMembers);
        ...
        //перебрать additionalKeyMembers и если есть quoted-printable конвертнуть entry.Value из hex в UTF8
        public static KeyValuePair<string,string> additionalKeyMembersProcessing(KeyValuePair<string, string> _entry,Dictionary<string,string> _additionalKeyMembers)
        {
            string value = _entry.Value;
            foreach (var item in _additionalKeyMembers)
            {
                ...
                switch (item.Value)
                {
                    case "QUOTED-PRINTABLE":
                            {
                                value = FsQuotedPrintable.DecodeString(_entry.Value, System.Text.Encoding.UTF8);
                            }
                            break;
                    default:
                        break;
            ...
            return new KeyValuePair<string, string>(_entry.Key, value);
        }
    }
}
